...
func (tc *TFJobController) reconcileTFJobs(tfjob *tfv1alpha2.TFJob) error {
	...
	if isSucceeded(tfjob.Status) || isFailed(tfjob.Status) {
		deleted := false
		// When CleanupPolicy is set, we ignore CleanPodPolicy. When
		// neither CleanupPolicy or CleanPodPolicy is set, we use
		...
		if tfjob.Spec.CleanupPolicy != nil || tfjob.Spec.CleanPodPolicy == nil || *tfjob.Spec.CleanPodPolicy == tfv1alpha2.CleanPodPolicy("") {
			if isAfterCleanDelay(tfjob) {
				// Clean the job by CleanPolicy and CleanDelay.
				if err := tc.doCleanup(tfjob); err != nil {
					return err
				}
				deleted = true
			}
